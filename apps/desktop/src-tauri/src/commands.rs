...
use aurora_launcher_core::{
    ...
    engine::{
        ...
        install_modrinth_content_with_progress as install_modrinth_project,
        ...
        InstanceLayout, IpcEvent, IpcServer, JavaRuntime, LaunchIdentity, LauncherEngine,
        MinecraftInstaller, ModpackInstallSummary, ModpackProgress, ModrinthContent,
        ModrinthSearchPage, ResolvedContentArtwork, SpeechResult, TransferProgress,
    },
};
...
pub fn launch_instance(
    ...
    if let Some(loader) = loader {
        install_embedded_companion(&instance, &minecraft_version, loader)
            .map_err(|error| error.to_string())?;
        let server = IpcServer::start().map_err(|error| error.to_string())?;
