...
/** Cliente WebSocket local, compatível com Java 8. */
final class AuroraIpcClient extends WebSocketClient {
    private static final long MINIMUM_INTERVAL_MS = 1000L;
    private final String nonce;
    private final AtomicLong lastTelemetryAt = new AtomicLong(0L);

    private AuroraIpcClient(URI endpoint, String nonce) { super(endpoint); this.nonce = nonce; }
    ...
    static AuroraIpcClient fromSystemProperties() {
        ...
        if (!nonce.matches("[0-9a-fA-F]{32}") || port < 1 || port > 65535) return null;
        try { return new AuroraIpcClient(new URI("ws://127.0.0.1:" + port + "/aurora"), nonce); }
        catch (URISyntaxException ignored) { return null; }
    }

    @Override public void onOpen(ServerHandshake handshake) {
        System.out.println("[Aurora] IPC local conectado.");
        send("{\"kind\":\"hello\",\"nonce\":\"" + nonce + "\",\"loader\":\"forge\",\"minecraftVersion\":\"1.12.2\"}");
    }
    @Override public void onMessage(String message) {
        if (message.contains("\"kind\":\"accepted\"")) System.out.println("[Aurora] Sessão IPC autenticada pelo launcher.");
        ...
        else AuroraAssistantOverlay.receive(message);
    }
    @Override public void onClose(int code, String reason, boolean remote) {
        System.out.println("[Aurora] IPC local desconectado (código " + code
            + ", origem " + (remote ? "launcher" : "jogo") + "): " + reason);
    }
